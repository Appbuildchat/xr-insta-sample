...
class HeartController(
    ...
    private fun loadHeartModel() {
        coroutineScope.launch {
            gltfModel = HeartGltfModelCache.getOrLoadModel(xrSession, context)
        }
    }
}

private object HeartGltfModelCache {
        ...
        return if (cachedModel == null) {
            try {
                val inputStream: InputStream =
                    context.resources.openRawResource(R.raw.heart)
                cachedModel = GltfModel.create(
                    xrCoreSession, inputStream.readBytes(), "HEART"
                )
                cachedModel
            } catch (e: Exception) {
    ...
    const val TAG = "HEARTGltfModelCache"
}